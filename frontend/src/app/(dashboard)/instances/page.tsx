  ...
  const onTest = useCallback(
    async (i: Instance) => {
      ...
      try {
        ...
        if (res.mode === "async") {
          setNotice(
            `Connection test queued for the agent on "${serverName.get(i.server_id ?? "") ?? "server"}" — see Operations.`,
          );
        } else if (res.ok) {
  ...
  const onImport = useCallback(
    async (i: Instance) => {
      ...
      try {
        ...
        if (res.mode === "async") {
          setNotice(
            "Import queued for the agent — discovered databases will appear shortly.",
          );
        } else {
          setNotice(
            `Imported ${res.imported} database${res.imported === 1 ? "" : "s"} from "${i.name}".`,
          );
        }
      } catch (err) {
  ...
  const columns = useMemo(
    (): DataTableColumn<Instance>[] => [
      ...
      },
      {
        ...
        render: (i) => (
          <span
            className={`badge ${i.kind === "external" ? "badge-amber" : "badge-gray"}`}
          >
            {i.kind}
          </span>
        ),
      },
      {
        ...
        header: "Engine",
        className: "muted",
        render: (i) => `${i.engine} ${i.engine_version}`,
      },
      {
        id: "location",
        header: "Location",
      ...
      },
      {
        ...
        header: "Credentials",
        className: "muted",
        render: (i) => (i.has_credentials ? "configured" : "—"),
      },
      {
        id: "status",
        header: "Status",
              ...
              <>
                ...
                {canWrite ? (
                  <button
                    className="btn btn-sm"
                    onClick={() => onImport(i)}
                    disabled={importDbs.isPending}
                    title="Import existing databases"
                  >
              ...
              <button
                ...
                onClick={() => {
                  if (i.provisioned) {
                    if (
                      !confirm(
                        `Remove provisioned instance "${i.name}"? Its Docker container will be stopped and removed.`,
                      )
                    )
                      return;
                    const removeVolume = confirm(
                      "Also delete the data volume? This permanently destroys the database data. Cancel to keep the volume.",
                    );
                    del.mutate({ id: i.id, removeVolume });
                  } else if (
                    confirm(
                      `Remove instance "${i.name}" from the control plane? The actual database server is not touched.`,
                    )
                  ) {
                    del.mutate({ id: i.id });
                  }
...
function AddInstanceModal({
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Failed to add instance",
      );
    }
  }
      ...
      <form onSubmit={onSubmit}>
        <Field label="Kind">
          <select
            className="input"
            value={kind}
            onChange={(e) => setKind(e.target.value as "managed" | "external")}
          >
        ...
        <p className="text-sm muted" style={{ marginTop: ".2rem" }}>
          Credentials are encrypted at rest and enable database provisioning,
          imports, backups and restores. Without them the instance is
          metadata-only.
        </p>
        ...
        >
          ...
          <button
            ...
            disabled={
              create.isPending || (kind === "managed" && servers.length === 0)
            }
          >
        ...
        {kind === "managed" && servers.length === 0 ? (
          <p className="text-sm muted">
            Connect a server first, or add an external instance.
          </p>
        ) : null}
      </form>
    </Modal>

...
- Bootstrap admin: credentials from `MDCP_ADMIN_EMAIL` / `MDCP_ADMIN_PASSWORD` in `.env` (generated by `generate-secrets.sh`)
...
Environment variables use the `MDCP_*` prefix (MariaDB Control Plane — historical
...
| `MDCP_ADMIN_EMAIL` / `MDCP_ADMIN_PASSWORD` | — | first-run bootstrap only; generate with `./scripts/generate-secrets.sh` |
...
- **Production mode:** set `MDCP_ENV=production` and provide strong values for
  `MDCP_JWT_SECRET`, `MDCP_ENCRYPTION_KEY`, and `MDCP_ADMIN_PASSWORD`. The API
